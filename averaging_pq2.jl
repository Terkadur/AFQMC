...
for (i, filling) in enumerate(filling_list)
    # merge data
    filelist = filter(x -> match(Regex("Pq2_LA3_Nup$(filling[1])_Ndn$(filling[2])_U2.0_beta18.0_*"), x) !== nothing, readdir(path_src))
    ...
    # convergence
    En_raw = reshape(reshape(possible_n, 1, :) * Pn2_raw, :)
    Em_raw = reshape(reshape(possible_m, 1, :) * Pm2_raw, :)
...
jldopen("$(path_dst)" * "Pq2_LA3_U2.0_beta18.0.jld", "w") do file
