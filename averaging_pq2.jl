...
U_list = [5.0] # collect(-8.0:1.0:8.0)
LA = 1
...
for (i, U) in enumerate(U_list)
    local file = "./data/test/Pq2_LA1_N2_U$(U)_beta90.0_dt0.1_seed1234.jld"
    local data = load(file)
    local Pmn2_raw, Pn2_raw, Pm2_raw = get_probs_raw(data)

    jldopen("./data/test/processed/Pq2_LA1_N2_U$(U)_beta90.0_dt0.1_seed1234.jld", "w") do file
        write(file, "Pm2", Pm2_raw)
    end

    # Pmn2_avg[:, :, i], Pmn2_err[:, :, i], Pn2_avg[:, i], Pn2_err[:, i], Pm2_avg[:, i], Pm2_err[:, i] = get_probs_stats(Pmn2_raw[:, :, :21000], Pn2_raw[:, :21000], Pm2_raw[:, :21000])
    # Hn2_avg[i], Hn2_err[i], Hm2_avg[i], Hm2_err[i] = get_shannon_stats(Pn2_raw, Pm2_raw)
end

# jldopen("./data/test/processed/Pq2_LA1_N2_beta18.0_seed1234_21000.jld", "w") do file
#     write(file, "U", U_list)
...
# jldopen("./data/two_site/processed/Hq2_LA1_N2_beta18.0_seed1234.jld", "w") do file
#     write(file, "U", U_list)
