...
function compute_Metropolis_ratio_asymmetric(
    ...
    GA⁻¹ = replica.GA⁻¹ # involved in Grover matrix
    a = replica.a # empty vector
    b = replica.b # empty vector
    λₖ = replica.λₖ # thermaldynamic integration variable
    Gτ_up = walker.G[1][sidx, sidx] # equal time spin up Green's at τ?
    Gτ_dn = walker.G[2][sidx, sidx] # equal time spin down Green's at τ?
