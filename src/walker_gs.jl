...
function HubbardWalker(
    ...
    Ns = system.V
    Np = system.N
    @assert size(φ₀[1]) == (Ns, Np[1]) && size(φ₀[2]) == (Ns, Np[2]) "Invalid trial wavefunction!"
    ...
    Ul_up = Matrix{T}(1.0I, Np[1], Ns)
    Ur_up = Matrix{T}(1.0I, Ns, Np[1])
    Ul_dn = Matrix{T}(1.0I, Np[2], Ns)
    Ur_dn = Matrix{T}(1.0I, Ns, Np[2])
    compute_G!(G[1], φ₀[1], φ₀T[1], Ul_up, Ur_up, Fl[1], Fr[1])
    compute_G!(G[2], φ₀[2], φ₀T[2], Ul_dn, Ur_dn, Fl[2], Fr[2])
    # G(τ=0, 0) = G(0)
    copyto!.(Gτ0, G)
    # G(0, τ=0) = G(0) - I
    copyto!.(G0τ, G)
